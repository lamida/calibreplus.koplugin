describe("Calibre library db module", function()
    ...
    setup(function()
        ...
        package.path = "plugins/calibreplus.koplugin/?.lua;" .. package.path
    ...
    it("filters by tag", function()
        local books = CalibreDB:queryAllBooks(library_dir)
        local matches = CalibreDB:filterBooks(books, "tag:fiction")
        -- Alpha and Beta have the fiction tag
        assert.are.equal(2, #matches)
    end)

    it("filters by tag and free text together", function()
        local books = CalibreDB:queryAllBooks(library_dir)
        local matches = CalibreDB:filterBooks(books, "tag:mystery alpha")
        assert.are.equal(1, #matches)
        assert.are.equal("Alpha", matches[1].title)
    end)
